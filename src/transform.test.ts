...
describe('Transform.updateMatrix()', () => {
  ...
  it('position [3, 0, 0] → elements[12] = 3', () => {
    ...
    t.position = [3, 0, 0];
    t.updateMatrix();
    assert.ok(near(t.modelMatrix.elements[12], 3), `expected 3, got ${t.modelMatrix.elements[12]}`);
  });
  ...
  it('position [0, 7, 0] → elements[13] = 7', () => {
    ...
    t.position = [0, 7, 0];
    t.updateMatrix();
    assert.ok(near(t.modelMatrix.elements[13], 7), `expected 7, got ${t.modelMatrix.elements[13]}`);
  });
  ...
  it('scale [2, 1, 1] → elements[0] = 2, other diagonal unchanged', () => {
    ...
    t.scale = [2, 1, 1];
    t.updateMatrix();
    assert.ok(near(t.modelMatrix.elements[0], 2), `sx: expected 2, got ${t.modelMatrix.elements[0]}`);
  ...
  it('translation + scale are both reflected in the matrix', () => {
    ...
    t.scale = [3, 1, 1];
    t.updateMatrix();
    assert.ok(near(t.modelMatrix.elements[0], 3), `sx: expected 3, got ${t.modelMatrix.elements[0]}`);
    assert.ok(near(t.modelMatrix.elements[12], 5), `tx: expected 5, got ${t.modelMatrix.elements[12]}`);
  });
